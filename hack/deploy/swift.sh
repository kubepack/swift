...
curl -fsSL https://raw.githubusercontent.com/appscode/swift/0.7.1/hack/deploy/server.yaml | $ONESSL envsubst | kubectl apply -f -
...
if [ "$SWIFT_ENABLE_RBAC" = true ]; then
    ...
    curl -fsSL https://raw.githubusercontent.com/appscode/swift/0.7.1/hack/deploy/rbac-list.yaml | $ONESSL envsubst | kubectl auth reconcile -f -
fi

if [ "$SWIFT_RUN_ON_MASTER" -eq 1 ]; then
    kubectl patch deploy swift -n $SWIFT_NAMESPACE \
      --patch="$(curl -fsSL https://raw.githubusercontent.com/appscode/swift/0.7.1/hack/deploy/run-on-master.yaml)"
fi
