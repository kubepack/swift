...
var Connectors = &connectorExt{
	newExtensionPoint(new(Connector)),
}

type connectorExt struct {
	*extensionPoint
}

func (ep *connectorExt) Unregister(name string) bool {
	return ep.unregister(name)
}

func (ep *connectorExt) Register(extension Connector, name string) bool {
	return ep.register(extension, name)
}

func (ep *connectorExt) Lookup(name string) Connector {
	ext := ep.lookup(name)
	if ext == nil {
		return nil
	}
...
func (ep *connectorExt) Select(names []string) []Connector {
...
func (ep *connectorExt) All() map[string]Connector {
	all := make(map[string]Connector)
...
func (ep *connectorExt) Names() []string {
	var names []string
	for k := range ep.all() {
